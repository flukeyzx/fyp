...
export async function getAllJobApplications(jobId, token) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/applications/${jobId}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
    ...
    if (!res.ok && res.status !== 404) {
      throw new Error(
        `${data.message || "Error while fetching job applications"}`
      );
    }

    return data;
  } catch (error) {
    console.log("Error while fetching job applications.", error.message);
    throw error;
  }
}

export async function analyzeResumeWithPrompt(prompt, resume, token) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/resume/analyze-with-prompt`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    const data = await res.json();
    if (!res.ok) {
      throw new Error(data.message || "Error while analyzing user's resume.");
    }

    return data;
  } catch (error) {
    console.log("Error while analyzing user's resume.", error.message);
    throw error;
  }
}

export async function getAtsScore(resume, description, token) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/resume/ats-score`,
    ...
    const data = await res.json();
    if (!res.ok) {
      throw new Error("Error while getting ATS score.");
    }

    return data;
  } catch (error) {
    console.log("Error while getting ATS score.", error.message);
    throw error;
  }
}
...
export async function updateApplicationStatus(applicationId, status, token) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/applications/${applicationId}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
...
export async function getTopFollowedCompanies(token) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/company/top-followed-companies`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    const data = await res.json();
    if (!res.ok && res.status !== 404) {
      throw new Error(data.message || "Error while fetching top companies.");
    }
    ...
    console.log("Error while fetching top companies.", error.message);
    throw error;
  }
}

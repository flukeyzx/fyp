...
export const getTopFollowedCompanies = async (_, res) => {
  try {
    const companies = await prisma.company.findMany({
      where: {
        followers: {
    ...
    return res.status(200).json({
      success: true,
      message: "Top hiring companies fetched successfully.",
      companies,
    });
  } catch (error) {
    console.log("Error in getTopHiringCompanies controller.", error.message);
    return res
      .status(500)
      .json({ success: false, message: "Internal server error." });
  }
